...
const indexOf = function(array, target, fromIndex=0) {
  for (let ind = fromIndex; ind < array.length; ind++) {
    if (array[ind] === target) {
      return ind;
    }
  }
...
const map = function(obj, callback=identity) {
  const results = [];
  each(obj, (currentValue, currentIndex, iteratedObj) => {
    results.push(callback(currentValue, currentIndex, iteratedObj));
  });
  return results;
};
...
const every = function(obj, callback=identity) {
  return reduce(obj, (wasAccepted, b) => {
    return wasAccepted && !!callback(b); 
  }, true);
};
...
const filter = function(obj, callback=identity) {
  const result = [];
  each(obj, (item) => {
    return !!callback(item) && result.push(item);
  });
  return result;
};
